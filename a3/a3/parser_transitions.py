...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        ###         2. Left Arc
        ###         3. Right Arc
        if transition==SHIFT and len(self.buffer)>0 :
            self.stack.append(self.buffer.pop(0))
        if transition==RIGHT_ARC and len(self.stack)>1:
            j=self.stack.pop()
            self.dependencies.append((self.stack[-1],j))
        if transition==LEFT_ARC and len(self.stack)>1:
            i=self.stack.pop(-2)
            self.dependencies.append((self.stack[-1],i))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    for s in sentences:
        ...
        dependencies.append(partial_parses[-1].dependencies)
    unfinished_parses = partial_parses[:]
    while len(unfinished_parses)>0:
        m=min(batch_size,len(unfinished_parses))
        steps=model.predict(unfinished_parses[:m])
        for j in range(m):
            unfinished_parses[j].parse_step(steps[j])
